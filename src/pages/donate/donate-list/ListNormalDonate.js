...
import {
	...
	getAllNormalDonates,
	rejectDonation,
	selectAllNormalDonates,
	selectDonateLoading,
} from "../../../services/slicer/DonateSlicer";
import { useEffect } from "react";

export const ListNormalDonate = () => {
	const navigate = useNavigate();
	const dispatch = useDispatch();
	const [form] = Form.useForm();

	const DonateLoading = useSelector(selectDonateLoading);
	const ListNormalDonate = useSelector(selectAllNormalDonates);
	...
	const onFinishAgree = async (value) => {
		...
		const confirm_data = {
			...
			status: "confirmed",
			transactionID: actionId,
			exact_amount: componentDisabled
				? currentAmount
				: value.exact_amount,
		};
	...
	const columns = [
		...
		},
		{
			...
			render: (send_amount) => (
				<>
					<Tooltip
						placement="topLeft"
						title={convertVNDMoney(send_amount)}
					>
						<p
							style={{
								color: "rgb(14 160 97)",
		...
		},
		{
			...
			render: (status, record) => <DonateStatus value={status} />,
		},
		{
			title: () => (
				<p
				...
				{DonateLoading === false ? (
					<>
						...
						>
							...
							/>
							<p>
								Bạn có thực sự muốn từ chối yêu cầu này không ?
						...
						>
							...
							/>
							<p>
								Bạn có chắc chắn xác nhận duyệt yêu cầu này
								không ? Nếu có hãy ghi chứ lời nhắn nhé
							</p>
							<Checkbox
									...
									rules={[
										{
											...
											message:
												"Bạn chưa ghi số tiền được nhận, nếu đã chính xác vui lòng tích phần duyệt số tiền",
										},
									]}
								>
									<Input
