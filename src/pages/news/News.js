...
import { deleteArticle, getAllArticles, getAllTopicAndFunds, selectAllArticles, selectAllTopicsAndFunds } from '../../services/slicer/ArticleSlicer';
import { Card, List, Row, Col, Button, Select, Input, Popover, Divider, Form,  Tag, Modal } from 'antd';
import './index.scss'
...
const News = () => {
  ...
  const renderTitle = (title) => {
    ...
    return lowerCaseTitle[0].toUpperCase() + lowerCaseTitle.slice(1).toLowerCase();
  }
  ...
  const showDeleteModal = (item) => {
    ...
    const handleDeleteArticle = async (value) => {
        console.log({value})
        try {
          await dispatch(deleteArticle({deleteId, topicIsSuggest })).unwrap();
    ...
    <div className='list-article__page'>
      ...
      <Row
                ...
                >
                    ...
                    >
                      <Popover placement="bottomLeft" title='Bộ lọc' content={content} trigger="click" 
                      overlayStyle={{
                        width : '400px'
                      }}>
                        <>
      ...
      <Modal title='Xác nhận xóa trung tâm' open={isModalOpen} onOk={handleDeleteArticle} onCancel={handleCancel}>
