...
import AuthSlice, {
	...
	selectCurrentUser,
	selectUserRole,
} from "../../services/slicer/AuthSlicer";
...
} from "../../services/slicer/NotificationSlicer";
...
const Navbar = () => {
	...
	const loadMoreData = async () => {
		...
		if (currentRole === 1) {
			...
			try {
				// await dispatch(getAllNotifications({currentRole, offset})).unwrap()
				let res = await getAllCenterNotificationsService(
					currentRole,
					centerId,
					offset
				);
	...
	const handleDirectToProfile = (event) => {
		event.preventDefault();
		if (currentRole === 1) {
			navigate("/profile");
		} else if (currentRole === 2) {
	...
	const items = [
		{
			label: <div onClick={handleDirectToProfile}>Thông tin cá nhân</div>,
			key: "0",
		},
		{
			label: (
						...
						<List
							...
							renderItem={(item) =>
								...
								item.read_at === null ? (
									<Link
										to={
											item.type.includes("request")
												? `/requests/${item.data.id}`
												: `/reports/${item.data.id}`
										}
										onClick={() =>
									...
									>
										...
										>
											<div className="notification--item__inner">
												{item.type === "request/new" ? (
													<span>
														<b>
															{item.center.name}
														</b>{" "}
														đã tạo yêu cầu mới, hãy
														bấm vào đế xem chi tiết
														yêu cầu
													</span>
												) : item.type ===
												  "request/moneyConfirmed" ? (
													<span>
														<b>
															{item.center.name}
														</b>{" "}
														đã xác nhận chuyển tiền,
														hãy bấm vào để xem chi
														tiết
													</span>
												) : item.type ===
												  "report/new" ? (
											...
											<div className="read__signal"></div>
										</List.Item>
									</Link>
								) : (
									<Link
                      ...
                      {item.type === "request/new" ? (
						...
						<List
							...
							renderItem={(item) => (
								// <NotificationsContent data={item}/>
								<Link
									to={item.type.includes('request') ? `/requests/${item.data.id}` : `/reports/${item.data.id}`}
									onClick={() =>
										handleClickNotification(
											item.read_at,
											item.id
										...
										<div className="notification--item__inner">
											...
											"request/accepted" ? (
												<span>
													...
													duyệt, Hãy bấm vào để xem và
													xác nhận nhận tiền
												</span>
											) : item.type ===
											  "request/rejected" ? (
												<span>
													Yêu cầu của bạn đã bị{" "}
													<b>từ chối</b>, Hãy bấm vào
													để xem chi tiết
												</span>
											) : item.type ===
											  "report/accepted" ? (
												<span>
													Có 1 báo cáo của bạn đã được
													duyệt, Hãy bấm vào để xem
													chi tiết
												</span>
											) : item.type ===
											  "report/rejected" ? (
												<span>
													Có 1 báo cáo của bạn đã bị{" "}
													<b>từ chối</b>, Hãy bấm vào
													để xem chi tiết
												</span>
											) : (
